...
async function main() {
  ...
  for (let originalPool of pools) {
    ...
    let strategy = await ( await hre.ethers.getContractFactory('ControllerBalancerV2Strat')).deploy(...args, {gasPrice: MIN_GAS_PRICE});
    ...
    for (let token of pool.rewards) {
      ...
      // Reward =>  => want
      await (await strategy.setRewardToWantRoute(token, [token, "0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270", pool.address], {gasPrice: MIN_GAS_PRICE})).wait()
    }
  }
}
