...
const deployController = async (token, archimedes) => {
...
describe('ArchimedesAPI', () => {
  ...
  describe('addNewPool', async () => {
    it('Should reverse with zero address want', async () => {
      ...
      expect(
        archimedes.addNewPool(zeroAddress, controller.address, 1)
      ).to.be.revertedWith('Address zero not allowed')
    })

    it('Should reverse with non-farm controller', async () => {
      const controller = await deploy('StratMock', owner.address, piToken.address)

      expect(
        archimedes.addNewPool(piToken.address, controller.address, 1)
  ...
  describe('changePoolWeighing', async () => {
    it('Should reverse with zero address want', async () => {
      const controller = deployController(piToken, archimedes)

      await archimedes.addNewPool(piToken.address, controller.address, 1)
  ...
  describe('pendingPiToken', async () => {
    beforeEach(async () => {
      const controller = deployController(piToken, archimedes)
      await waitFor(archimedes.addNewPool(piToken.address, controller.address, 1))
  ...
  describe('depositMATIC', async () => {
    ...
    beforeEach(async () => {
      controller = deployController(WMATIC, archimedes)

      await (await archimedes.addNewPool(WMATIC.address, controller.address, 1)).wait()
    })
    ...
    it('Should get wmatic shares and then withdraw', async () => {
      ...
      expect(await controller.balanceOf(owner.address)).to.be.equal(toNumber(1e18))
      ...
      expect(await controller.balanceOf(owner.address)).to.be.equal(0)
  ...
  describe('withdraw', async () => {
    beforeEach(async () => {
      const controller = await deploy('StratMock', archimedes.address, piToken.address)
      await controller.deployed()
      await waitFor(archimedes.addNewPool(piToken.address, controller.address, 1))
    })
  ...
  describe('getPricePerFullShare', async () => {
    ...
    beforeEach(async () => {
      controller = await deploy('StratMock', archimedes.address, piToken.address)
      await controller.deployed()
      await waitFor(archimedes.addNewPool(piToken.address, controller.address, 1))
    })
