...
describe('Controller Curve Stable Strat', () => {
  let bob
  let piToken
  let archimedes
  let controller
  ...
  beforeEach(async () => {
    ...
    controller   = await createController(USDC, archimedes, 'ControllerCurveStableStrat')
    strat        = await ethers.getContractAt('ControllerCurveStableStrat', (await controller.strategy()))
...
describe('Controller Curve Stable Strat with DAI', () => {
  let bob
  let piToken
  let archimedes
  let controller
  ...
  beforeEach(async () => {
    ...
    REWARD_TOKEN = await ethers.getContractAt('IERC20Metadata', CRV.address)
    controller   = await createController(DAI, archimedes, 'ControllerCurveStableStrat')
    strat        = await ethers.getContractAt('ControllerCurveStableStrat', (await controller.strategy()))
